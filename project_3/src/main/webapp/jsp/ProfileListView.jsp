    ...
    %>
		...
		<form action="<%=ORSView.PROFILE_LIST_CTL%>" method="post">
            ...
            List<ProfileDTO> list =
                (List<ProfileDTO>) ServletUtility.getList(request);
					...
					<tr>
						<td>
							<div
								...
								<div class="mx-2">
									<label><b>Profile Status :</b></label>
								</div>
								<div class="mx-2">
									<%=HTMLUtility.getList("profileStatus", String.valueOf(dto.getProfileStatus()), profileStatusMap)%>
								</div>
								<div class="mx-2">
									<input type="submit" class="btn btn-sm btn-primary"
										name="operation" value="<%=ProfileListCtl.OP_SEARCH%>">
									<input type="submit"
			...
			<div class="table-responsive">
				<table
					...
					<tbody>
						<%
                    while (it.hasNext()) {
                        dto = (ProfileDTO) it.next();
                %>
						<tr>
							<td><input type="checkbox" class="case" name="ids"
								value="<%=dto.getId()%>"></td>
							<td><%=index++%></td>
							<td class="text-capitalize"><%=dto.getFullName()%></td>
							<td class="text-capitalize"><%=DataUtility.getDateString(dto.getDob())%></td>
							<td class="text-capitalize"><%=dto.getGender()%></td>
							<td class="text-capitalize"><%=dto.getProfileStatus()%></td>
							<td><a href="ProfileCtl?id=<%=dto.getId()%>"
			...
			<table class="table w-100">
				<tr>
					<td class="text-right"><input type="submit"
						...
						value="<%=ProfileListCtl.OP_BACK%>"></td>
				</tr>
			</table>
