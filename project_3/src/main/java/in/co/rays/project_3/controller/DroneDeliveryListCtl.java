...
@WebServlet(urlPatterns = { "/ctl/DroneDeliveryListCtl" })
public class DroneDeliveryListCtl extends BaseCtl {
	...
	protected void preload(HttpServletRequest request) {
		...
		try {
			...
			request.setAttribute("droneList", list);
		} catch (Exception e) {
			log.error(e);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					DroneDeliveryDTO deletedto = new DroneDeliveryDTO();
					for (String id : ids) {
			...
			dto = (DroneDeliveryDTO) populateDTO(request);
