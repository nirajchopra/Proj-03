 ...
 *
 */
@WebServlet(name = "RoleListCtl", urlPatterns = { "/ctl/SchedularJobListCtl" })
public class SchedularJobListCtl extends BaseCtl {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static Logger log = Logger.getLogger(SchedularJobListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("SchedularJobListCtl doGet Start");
		List list = null;
		List next = null;
		...
		log.debug("SchedularJobListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		log.debug("SchedularJobListCtl doPost Start");
		List list = null;
		List next = null;
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					SchedulerJobDTO deletebean = new SchedulerJobDTO();
					for (String id : ids) {
			...
			dto = (SchedulerJobDTO) populateDTO(request);
			list = model.search(dto, pageNo, pageSize);
		...
		log.debug("SchedularJob List doPost End");
	}

	@Override
	protected String getView() {
		return ORSView.SCHEDULAR_JOB_LIST_VIEW;
	}
