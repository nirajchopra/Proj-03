...
@WebServlet(name = "ECommerceListCtl", urlPatterns = { "/ctl/ECommerceListCtl" })
public class ECommerceListCtl extends BaseCtl{
	
	private static final long serialVersionUID = 1L;
	private static Logger log = Logger.getLogger(ECommerceListCtl.class);

	protected void preload(HttpServletRequest request) {
		ECommerceModelInt model = ModelFactory.getInstance().getECommerceModel();
		try {
			List list = model.list();
			request.setAttribute("ecoList", list);
		} catch (Exception e) {
			log.error(e);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("ECommerceListCtl doGet Start");
		List list;
		List next;
		int pageNo = 1;
		...
		log.debug("ECommerceListCtl doPOst End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					ECommerceDTO deletedto = new ECommerceDTO();
					for (String id : ids) {
			...
			dto = (ECommerceDTO) populateDTO(request);
		...
		log.debug("ECommerceListCtl doGet End");
	}

	@Override
	protected String getView() {
		return ORSView.ECOMMERCE_LIST_VIEW;
	}
