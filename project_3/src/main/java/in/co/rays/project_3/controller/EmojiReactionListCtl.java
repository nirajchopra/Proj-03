...
@WebServlet(name = "EmojiReactionListCtl", urlPatterns = { "/ctl/EmojiReactionListCtl" })
public class EmojiReactionListCtl extends BaseCtl {
	...
	protected void preload(HttpServletRequest request) {
		...
		try {
			...
			request.setAttribute("emojiList", list);
		} catch (Exception e) {
			log.error(e);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					EmojiReactionDTO deletedto = new EmojiReactionDTO();
					for (String id : ids) {
			...
			dto = (EmojiReactionDTO) populateDTO(request);
	...
	protected String getView() {
		return ORSView.EMOJI_REACTION_LIST_VIEW;
	}
